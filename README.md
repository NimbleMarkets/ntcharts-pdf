...
Page with `n` / `p` (or arrows), toggle Text ↔ Image with `m`, swap Glyph ↔ Kitty with `g`. Bindings live in `pdfview.DefaultKeyMap()` and can be rebound field-by-field.
...
| `pdfview.ImageMode` | Rasterized page via a `Renderer` (default: `go-pdfium` running PDFium as WASM via wazero), fed to `ntcharts/v2/picture` | Native out of the box; WASM target needs a custom `RendererFactory` |
...
- **Per-page text extraction failures are recorded inline** rather than failing the whole load. A page that ledongthuc/pdf can't decode (rare — usually encrypted font tables) shows up as `[page N: <error>]` in TextMode and as a blank in ImageMode.
