...
// repoTestdata locates testdata/ at the repo root regardless of where the
// test runs. Tests run with WD = the package dir, so the conventional
// in-package testdata/ lookup would miss the repo-root samples; this
// helper bridges that without forcing a binary blob copy.
func repoTestdata(t *testing.T, name string) string {
	t.Helper()
	for _, rel := range []string{"testdata/" + name, "../testdata/" + name} {
